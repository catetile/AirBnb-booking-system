...
if($_GET['id']){
	...
	if($room_id > 0){
	$room = $conn->query("SELECT * FROM airbnbs where id =".$room_id)->fetch_array();
	$cat = $conn->query("SELECT * FROM airbnb_categories where id =".$room['category_id'])->fetch_array();
}else{
	$cat = $conn->query("SELECT * FROM airbnb_categories where id =".$booked_cid)->fetch_array();
...
<div class="container-fluid">
	<p><b>Room : </b><?php echo isset($airbnb['airbnb']) ? $airbnb['airbnb'] : 'NA' ?></p>
